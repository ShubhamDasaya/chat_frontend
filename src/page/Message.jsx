...
import { Check, CheckCheck, Pencil, Trash2, Copy, CheckCircle, Trash, Share2 } from "lucide-react";
import Avatar, { getAvatarURL } from "../ui/Avatar";
import Lightbox from "../ui/Lightbox";

const quickEmojis = ["👍", "❤️", "😂", "😮", "😢", "🙏"];

// Safe Markdown + Link Parser
const parseMarkdown = (text) => {
    ...
    escaped = escaped.replace(
        /(https?:\/\/[^\s<>"']+)/g,
        '<a href="$1" target="_blank" rel="noopener noreferrer" class="underline underline-offset-2 opacity-90 hover:opacity-100 break-all" onclick="event.stopPropagation()">$1</a>'
    );
...
const Message = ({
    ...
    return (
        <>
            {lightboxSrc && (
                <Lightbox src={lightboxSrc} alt="Image" onClose={() => setLightboxSrc(null)} />
            )}
            ...
            >
                ...
                >
                    ...
                    <div
                        ...
                        className={`relative px-3.5 py-2.5 rounded-2xl cursor-pointer select-text transition-all hover:scale-[1.01] duration-200 ease-out
                        ...
                        style={deleted
                            ...
                            : own
                                ? { background: "linear-gradient(135deg, var(--msg-own-from), var(--msg-own-to))", color: "var(--msg-own-text)", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 4px 16px rgba(99,102,241,0.25), inset 0 1px 0 rgba(255,255,255,0.1)" }
                                : { background: "var(--msg-other-bg)", color: "var(--msg-other-text)", border: "1px solid var(--msg-other-border)", boxShadow: "0 4px 16px rgba(0,0,0,0.1)" }
                        }
                    >
                        {/* Quoted Reply Banner */}
                                    ...
                                    if (type === "image") {
                                        return (
                                            <img
                                                key={idx}
                                                src={fullUrl}
                                                className="max-w-full max-h-60 rounded-xl object-contain cursor-zoom-in border border-white/10 shadow-md hover:brightness-90 transition-all"
                                                alt="Media Attachment"
                                                onClick={(e) => { e.stopPropagation(); setLightboxSrc(fullUrl); }}
                                                loading="lazy"
                                            />
                                        );
                                    } else if (type === "video") {
                        ...
                        {message && (
                            deleted ? (
                                <div className="text-[13.5px] leading-[1.5] italic" style={{ wordBreak: "break-word", overflowWrap: "anywhere", whiteSpace: "pre-wrap" }}>
                                ...
                                <div
                                    ...
                                    style={{ wordBreak: "break-word", overflowWrap: "anywhere", whiteSpace: "pre-wrap", paddingRight: "56px" }}
                        ...
                        {/* Date + time tooltip on hover */}
                        {!deleted && (
                            <div
                                className="absolute -top-7 left-1/2 -translate-x-1/2 px-2 py-1 rounded-md text-[9px] text-[var(--text-muted)] opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-40"
                    ...
                    {!deleted && (hovered || showActions || copied) && (
                        ...
                        >
                            ...
                            {onForward && (
                                <button
                                    onClick={(e) => { e.stopPropagation(); onForward({ id, message, media, mediaType }); setShowActions(false); }}
                                    className="flex items-center gap-1 text-[10px] font-bold text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors pr-1 border-r border-[var(--glass-border)] mr-1"
                                >
                                    <Share2 size={11} /> Fwd
                                </button>
                            )}
                    ...
                    {/* Seen indicator */}
                    {own && seen && !deleted && (
                        <span className="text-[9px] font-semibold leading-none mt-1 mr-1" style={{ color: "var(--text-muted)" }}>
                            Seen
                        </span>
                    )}
